...
public class Aerolinea implements Serializable
{
    ...
    {
        // TODO implementar
    	ArrayList<Tiquete> lisTiquetes = new ArrayList<Tiquete>();
    	for (Vuelo vuelo : this.vuelos ) {
    		List<Tiquete> valores = (List<Tiquete>) vuelo.getTiquetes();
    		lisTiquetes.addAll(valores);
    	}
        return lisTiquetes;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	for (Vuelo vuelo : vuelos) {
    		if (vuelo.getAvion().getNombre().equals(nombreAvion) && vuelo.getFecha().equals(fecha)) {
    			throw new Exception("El avión seleccionado está ocupado para otro vuelo en el mismo intervalo de tiempo");
    		}
    	}
    	
    	Avion avionProgramar = null;
    	for (Avion avion : aviones) {
    		if (avion.getNombre().equals(nombreAvion)) {
    			avionProgramar = avion;
    		}
    	}
    	
    	Vuelo vueloProgramar = new Vuelo(rutas.get(codigoRuta), fecha, avionProgramar);
    	vuelos.add(vueloProgramar);
    }
    ...
    {
        // TODO Implementar el método
    	Vuelo vueloRegistrar = getVuelo(codigoRuta, fecha);
    	
    	for (Tiquete tiquete : vueloRegistrar.getTiquetes()) {
